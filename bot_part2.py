...
async def cmd_status(msg: Message, state: FSMContext):
    ...
    await msg.answer(f"Holat: <code>{cur}</code>", parse_mode="HTML")
...
async def m_captcha(msg: Message, state: FSMContext):
    ...
    if status == 200 and isinstance(body, dict) and body.get("result"):
        ...
        try:
            nxt = body.get("captcha")
            if nxt:
                api.request_id = api.request_id  # keyingi captcha backend dan keladi
        except Exception:
            pass
        await wait.delete()
